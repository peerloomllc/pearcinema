...
// would have left the latest release with no APK at all. The helper is
...
function fakeRelease ({ apk = 'pearcinema-v1.1.4.apk', goodSum = true, withSum = true } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'carry-'))
  const files = path.join(dir, 'files')
  fs.mkdirSync(files)
  const assets = [{ name: 'PearCinema-1.1.4.dmg', browser_download_url: `file://${files}/PearCinema-1.1.4.dmg` }]
  if (apk) {
  ...
  fs.writeFileSync(path.join(api, 'latest'), JSON.stringify({ tag_name: 'v1.1.4', assets }))
  return dir
}
...
test('the previous APK and its checksum come across, under their original names', () => {
  ...
  const out = carry(dir)
  assert.deepEqual(out.map(p => path.basename(p)), ['pearcinema-v1.1.4.apk', 'pearcinema-v1.1.4.apk.sha256'])
...
function hasOwnApk (dir, version) {
  const r = spawnSync('bash', ['-c', `set -euo pipefail\n${ownApk}\nif _github_latest_has_own_apk "" o/r ${version}; then echo yes; else echo no; fi`], {
    env: { ...process.env, GITHUB_API: `file://${dir}/api`, ARTIFACT_PREFIX: 'pearcinema' }
  })
  assert.equal(r.status, 0, String(r.stderr))
...
test('a release with its own APK is a real build; a carried APK is not', () => {
  const dir = fakeRelease()
  assert.equal(hasOwnApk(dir, '1.1.4'), 'yes')
  assert.equal(hasOwnApk(dir, '1.1.5'), 'no')
})

test('a failed GitHub query keeps the old Zapstore shortcut', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'carry-'))
  assert.equal(hasOwnApk(dir, '1.1.5'), 'yes')
})
