...
function harness () {
  ...
  const m = createMethods({ getAdapter: () => adapter, getLibraryName: () => 'L', state })
  const ctx = (params = {}) => ({
    params,
    owner: 'p:ada',
    deviceKey: 'phone-1',
    ...
    presence: { notifyOwner: (owner, kind, data) => { owners.push({ owner, kind, data }); return 1 } },
    badParams: (x) => new Error(x),
    notFound: (x) => new Error(x),
    forbidden: (x) => new Error(x)
  })
  return { m, ctx, pushes, owners }
}
