...
_appstore_latest_version() {
  local bundle_id="${1:-${BUNDLE_ID:-com.pearcinema}}"
...
_host_image_latest_version() {
  local image="${1:-ghcr.io/peerloomllc/pearcinema-host}"
  local host="${image%%/*}"   # ghcr.io
  local repo="${image#*/}"    # peerloomllc/pearcinema-host
...
if [ -z "$ZSP_IDENTIFIER" ]; then
  ZSP_IDENTIFIER="${BUNDLE_ID:-com.pearcinema}"
...
ASC_VERSION_CURRENT=$(_appstore_latest_version "${BUNDLE_ID:-com.pearcinema}")
HOST_IMAGE="${HOST_IMAGE:-ghcr.io/peerloomllc/pearcinema-host}"
...
if $CHECK_VERSIONS_ONLY; then
  ...
  echo ""
  echo "    Start9 / StartOS: not a release channel for PearCinema (there is no start9/ tree here)."
  exit 0
fi
...
# step 2 only prebuilds it when it is absent - so on the normal path build.gradle
# is whatever the last prebuild wrote, not something regenerated from the app.json
# rewritten just above. build.gradle reads both values from the environment, via
# plugins/with-android-version-from-env.js. Exporting them is what makes the APK
# carry $APP_VERSION either way: a stale generated tree and a fresh one produce
# the same numbers, which is the point of reading them from the environment
# rather than baking them in at prebuild time.
...
#
# ios/ is GENERATED here and not committed, the same way android/ is. Step 2
# prebuilds it when the App Store is a destination, so this file is present by
# then and the sed runs. The existence check stays because a tree can legitimately
# lack it - an Android-only release never prebuilds ios/ at all, or a checkout
...
if ! $SKIP_DESKTOP; then
  ...
  if ! $SKIP_LINUX || ! $SKIP_WINDOWS; then
    ( cd "$_DT" && npm install --no-audit --no-fund --loglevel=error ) \
      > /tmp/pearcinema-build-deps.log 2>&1 || true
  fi
  ...
  if $SKIP_MACOS; then
    ...
    ( cd "$_DT" && ${DESKTOP_BUILD_MAC:-npm run build:mac} ) > /tmp/pearcinema-build-macos.log 2>&1 &
    _MAC_PID=$!
  fi
  ...
  if $SKIP_LINUX; then
    ...
    if ( cd "$_DT" && ${DESKTOP_BUILD_LINUX:-npm run build:linux} ) > /tmp/pearcinema-build-linux.log 2>&1; then
      for _f in "$_DT/dist/PearCinema-${_DTV}.AppImage" \
                "$_DT/dist/pearcinema-desktop_${_DTV}_amd64.deb"; do
        if [ -f "$_f" ]; then DESKTOP_ARTIFACTS+=("$_f"); fi
      done
      echo "    Linux build OK."
    else
      echo "    WARNING: Linux desktop build failed, skipping it. Log: /tmp/pearcinema-build-linux.log"
    fi
  fi
  ...
  if $SKIP_WINDOWS; then
    ...
    if ( cd "$_DT" && ${DESKTOP_BUILD_WINDOWS:-npm run build:windows} ) > /tmp/pearcinema-build-windows.log 2>&1; then
      ...
      _raw="$_DT/dist/PearCinema Setup ${_DTV}.exe"
      _f="$_DT/dist/PearCinema-Setup-${_DTV}.exe"
      [ -f "$_raw" ] && mv -f "$_raw" "$_f"
      [ -f "${_raw}.blockmap" ] && mv -f "${_raw}.blockmap" "${_f}.blockmap"
      if [ -f "$_f" ]; then DESKTOP_ARTIFACTS+=("$_f"); fi
      echo "    Windows build OK."
    else
      echo "    WARNING: Windows desktop build failed, skipping it. Log: /tmp/pearcinema-build-windows.log"
    fi
  fi
  ...
  if [ -n "$_MAC_PID" ]; then
    ...
    if wait "$_MAC_PID"; then
      for _f in "$_DT/dist/PearCinema-${_DTV}.dmg" \
                "$_DT/dist/PearCinema-${_DTV}-arm64.dmg"; do
        if [ -f "$_f" ]; then DESKTOP_ARTIFACTS+=("$_f"); fi
      done
      echo "    macOS build OK."
    else
      echo "    WARNING: macOS desktop build failed, skipping it. Log: /tmp/pearcinema-build-macos.log"
    fi
  fi
...
if $SKIP_HOST; then
  ...
  if [ -z "$HOST_IMAGE_BUILT" ]; then
    ...
    echo "    Full log: /tmp/pearcinema-build-host.log"
    ...
    if [ "$_host_status" -eq 0 ]; then
      echo "    Host image OK ($(grep -oE 'sha256:[0-9a-f]+' /tmp/pearcinema-build-host.log | tail -1)). Log: /tmp/pearcinema-build-host.log"
      echo "    Every file naming the image is now pinned to $HOST_IMAGE_BUILT — step 6 commits them."
    else
      HOST_IMAGE_BUILT=""
      echo "    WARNING: host image build/push failed, skipping it. Log: /tmp/pearcinema-build-host.log"
    fi
  fi
fi
...
if ! $PUBLISH_PLAY; then
  ...
  PLAY_PACKAGE="${BUNDLE_ID:-com.pearcinema}"
...
if [ -n "${UMBREL_STORE_DIR:-}" ] && [ -d "${UMBREL_STORE_DIR}/.git" ]; then
  _store_dirty=$(git -C "$UMBREL_STORE_DIR" status --porcelain -- '*pearcinema*' 2>/dev/null)
  ...
  if [ -n "$_store_dirty" ]; then
    ...
    echo "=========================================================================="
    echo ""
    echo "  $UMBREL_STORE_DIR has uncommitted PearCinema changes, so the store still"
    ...
    if [ "$_store_branch" != "$_store_default" ]; then
      ...
      echo "  Move the PearCinema changes onto a branch cut from origin/$_store_default."
      echo ""
    fi
