#!/bin/bash
# Debian postinst fragment - installs and starts the PearCinema host as a systemd
...
#
# Ported from peartune/desktop/installer/linux/postinst.sh, minus its privileged
# auto-updater (PearCinema has no update-apply yet).
#
...
INSTALL_ROOT=/opt/PearCinema
BIN="$INSTALL_ROOT/pearcinema-desktop"
HOST_ENTRY="$INSTALL_ROOT/resources/app.asar/src/main/service-entry.js"
UNIT_SRC="$INSTALL_ROOT/resources/pearcinema-host.service"
UNIT_NAME=pearcinema-host.service
...
if [ -z "$TARGET_USER" ] || [ "$TARGET_USER" = "root" ]; then
  echo "pearcinema: no non-root install user detected; skipping service setup."
  echo "  To run PearCinema as an always-on service, as your normal user:"
  echo "    pearcinema-desktop --install-service"
  exit 0
fi

if [ ! -f "$UNIT_SRC" ] || [ ! -x "$BIN" ]; then
  echo "pearcinema: install layout not as expected; skipping service setup."
  exit 0
fi
...
# Remember who we set this up for. postrm needs it, and a removal through a
...
if runuser -u "$TARGET_USER" -- systemctl --user enable --now "$UNIT_NAME" 2>/dev/null; then
  echo "pearcinema: host service enabled and started for $TARGET_USER."
  echo "  Dashboard: http://127.0.0.1:8751"
else
  echo "pearcinema: unit installed but could not be started right now."
