...
#
#   * /opt/PearCinema IS ALREADY GONE by the time this runs, so the .install-user
...
#
# THE DATA DIR IS NEVER TOUCHED. host.seed under ~/.config/pearcinema-desktop/data
...
cleanup_user () {
  ...
  # user manager running for someone who no longer has PearCinema installed.
  loginctl disable-linger "$user" 2>/dev/null || true

  echo "pearcinema: host service removed for $user."
  echo "  Your library data is untouched in $home/.config/pearcinema-desktop/data"
}
