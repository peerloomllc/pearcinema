...
//
// android/ is GENERATED AND GITIGNORED here, unlike the donor's, so this plugin is the
// only place the signing config exists - there is no committed build.gradle to keep in
// step. It still has to be idempotent, because prebuild runs against an
// already-generated tree when one is lying around; hence the guards below.
//
// PearCinema was without it until 2026-08-27, and it went unnoticed because a
// debug-signed release APK builds, installs and runs perfectly. What found it was
// filling in scripts/.env, generating the pearcinema key and then CHECKING what the
// release APK was actually signed with - which was CN=Android Debug.
...
// THE WHOLE TRANSFORM, as a function of the gradle text - so it can be tested without
// expo's mod machinery, which is the half that is awkward to drive from a test and the
// half that never breaks. What breaks is these two anchors, when Expo edits its template.
